...
const Appointment = () => {
  ...
  return docInfo && (
    <div>
      {/* -------------Doctor Details---------- */}
      <div className='flex felx-col sm:felx-row gap-4'>
        <div>
      ...
      <div className='sm:ml-72 sm:pl-4 mt-4 font-medium text-gray-700'>
        ...
        <div className='flex items-center gap-3 w-full overflow-x-scroll mt-4'>
          {docSlots.length && docSlots[slotIndex]
            .filter(item => item.time) // Only slots with a time property
            .map((item, index) => (
              <p
                onClick={() => setSlotTime(item.time)}
                className={`text-sm font0light flex-shrink-0 px-5 py-2 rounded-full cursor-pointer ${item.time === slotTime ? 'bg-primary text-white' : 'text-gray-400 border border-gray-300'}`}
                key={index}
              >
